...
app.get("/getAllNotes", express.json(), async (req, res) => {
  try{
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // Find note with given ID
      const collection = db.collection(COLLECTIONS.notes);
      const findNotes = await collection.find({
        username: decoded.username
      }).toArray();
      if (findNotes) {
        return res
          .status(200)
          .json({ response: findNotes });
      }
      // for await (const allNotes of findNotes) {
  ...
  try{
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (!title && !content) {
        return res
          .status(400)
          .json({ error: "Please ensure either title or content is being updated"});
      }
